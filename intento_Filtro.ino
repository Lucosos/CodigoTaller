...
float datos(int pin) {
  for (int i=0 ; i<10;i++){
    DatosMercurio[i]=digitalRead(pin);
    i+=1;
    delay(3);
    //PromedioMercurio=mean(DatosMercurio);
    
    }
  float PromedioMercurio=mean(DatosMercurio,10);
  return PromedioMercurio;
  ...
  while( Todo_esta_bien ){
    ...
    if (nroFase != (nroFases - 1) && nroFase != 0 && nroFase != 1   ){ 
     ...
     if (Fases[nroFase].tipo==4){//acelerado
      ...
      posLeftHip =   Fases[6].posMotores[0] + t*( Fases[nroFase].posMotores[0]-Fases[8].posMotores[0]) ; 
      posLeftKnee=   Fases[6].posMotores[1] + t*( Fases[nroFase].posMotores[1]-Fases[8].posMotores[1]) ; //nroFases -1 = ultimo 
      posLeftAnkle = Fases[6].posMotores[2] + t*( Fases[nroFase].posMotores[2]-Fases[8].posMotores[2]) ;
      posRightHip=   Fases[6].posMotores[3] + t*( Fases[nroFase].posMotores[3]-Fases[8].posMotores[3]) ;
      posRightKnee = Fases[6].posMotores[4] + t*( Fases[nroFase].posMotores[4]-Fases[8].posMotores[4]) ;   
      posRightAnkle =Fases[6].posMotores[5] + t*( Fases[nroFase].posMotores[5]-Fases[8].posMotores[5]) ;
      ...
      if(t>=1){
        nroFase=3; // ANTES ERA EN 1 y luego en 2
        Todo_esta_bien=false;
        k++;
      }
